/**
 * SecretTextView by Matt Kula (https://github.com/matthewrkula/SecretTextView)
 * Created by matt on 5/27/14.
 */

package com.yemyatthu.iymb.ui.widget;
  ...
  private void resetSpannableString(double percent) {
    ...
    for (int i = 0; i < mSpannableString.length(); i++) {
      mSpannableString.setSpan(new ForegroundColorSpan(
          Color.argb(clamp(mAlphas[i] + percent), Color.red(color), Color.green(color),
              Color.blue(color))), i, i + 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
